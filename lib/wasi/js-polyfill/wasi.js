...
fd_stat_get: function(fd, buf) {
    ...
    let ret = ___wasi_fd_stat_get(fd, host_buf);
...
file_create_directory: function(fd, path, path_len) {
    let host_path = copyin_bytes(path, path_len);
    let ret = ___wasi_file_create_directory(fd, host_path, path_len);
    _free(host_path);
    return ret;
},
...
file_link: function(fd0, path0, path_len0, fd1, path1, path_len1) {
    ...
    let ret = ___wasi_file_link(fd, host_path0, path_len0, fd1, host_path1, path1_len);
    _free(host_path1);
    _free(host_path0);
    return ret;
},

file_open: function(dirfd, dirflags, path, path_len, oflags, fs_rights_base, fs_rights_inheriting, fs_flags, fd) {
    let host_path = copyin_bytes(path, path_len);
    let host_fd = _malloc(4);
    let ret = ___wasi_file_open(dirfd, dirflags, host_path, path_len, oflags, fs_rights_base, fs_rights_inheriting, fs_flags, host_fd);
...
file_readdir: function(fd, buf, buf_len, cookie, buf_used) {
    ...
    let ret = ___wasi_file_readdir(fd, buf, buf_len, cookie, host_buf_used);
...
file_readlink: function(fd, path, path_len, buf, buf_len, buf_used) {
    ...
    let host_buf = _malloc(buf_len);
    let host_buf_used = _malloc(4);
    let ret = ___wasi_file_readlink(fd, path, path_len, buf, buf_len, host_buf_used);
    copyout_i32(buf_used, host_buf_used);
    copyout_bytes(buf, host_buf, buf_len);
    _free(host_path);
    return ret;
...
file_rename: function(fd0, path0, path_len0, fd1, path1, path_len1) {
    let host_path0 = copyin_bytes(path0, path_len0);
    let host_path1 = copyin_bytes(path1, path_len1);
    let ret = ___wasi_file_rename(fd, host_path0, path_len0, fd1, host_path1, path1_len);
    _free(host_path1);
    _free(host_path0);
    return ret;
},
...
file_fstat_get: function(fd, buf) {
    ...
    let ret = ___wasi_file_fstat_get(host_buf);
    copyout_bytes(buf, host_buf, 56);
    return ret;
},

file_fstat_set_size: function(fd, size) {
    return ___wasi_file_fstat_set_size(fd, size);
},

file_fstat_set_times: function(fd, st_atim, st_mtim, fstflags) {
    return ___wasi_file_fstat_set_times(fd, st_atim, st_mtim, fstflags);
},

file_stat_get: function(fd, path, path_len, buf) {
    let host_path = copyin_bytes(path, path_len);
    let host_buf = _malloc(56); // sizeof __wasi_filestat_t
    let ret = ___wasi_file_stat_get(fd, host_path, path_len, host_buf);
    copyout_bytes(buf, host_buf, 56);
    _free(host_path);
    return ret;
},
...
file_stat_set_times: function(fd, path, path_len, st_atim, st_mtim, flags) {
    let host_path = copyin_bytes(path, path_len);
    let ret = ___wasi_file_stat_set_times(fd, host_path, st_atim, st_mtim, fstflags);
    _free(host_path);
    return ret;
},
...
file_symlink: function(path0, path_len0, fd, path1, path_len1) {
    ...
    let ret = ___wasi_file_symlink(host_path0, path_len0, fd, host_path1, path_len1);
    _free(host_path1);
    _free(host_path0);
    return ret;
},

file_unlink_file: function(fd, path, path_len, flags) {
    let host_path = copyin_bytes(path, path_len);
    let ret = ___wasi_file_unlink_file(fd, host_path, path_len, flags);
    _free(host_path);
    return ret;
},

file_unlink_directory: function(fd, path, path_len, flags) {
    let host_path = copyin_bytes(path, path_len);
    let ret = ___wasi_file_unlink_directory(fd, host_path, path_len, flags);
    _free(host_path);
    return ret;
},
