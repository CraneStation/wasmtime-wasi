...
file_filestat_get: function(fd, path, path_len, buf) {
    let host_path = copyin_bytes(path, path_len);
    let host_buf = _malloc(56); // sizeof __wasi_filestat_t
    let ret = ___wasi_file_filestat_get(fd, host_path, path_len, host_buf);
    copyout_bytes(buf, host_buf, 56);
    _free(host_path);
    return ret;
},

file_filestat_set_times: function(fd, path, path_len, st_atim, st_mtim, flags) {
    let host_path = copyin_bytes(path, path_len);
    let ret = ___wasi_file_filestat_set_times(fd, host_path, st_atim, st_mtim, fstflags);
    _free(host_path);
    return ret;
},
